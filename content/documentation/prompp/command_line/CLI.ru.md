...
| `--web.config.file` | [ЭКСПЕРИМЕНТАЛЬНО] Путь к файлу конфигурации TLS/аутентификации |                                                                 |
...
| `--web.external-url` | Внешний URL для доступа к Deckhouse Prom++ (через reverse proxy) |                                                                 |
| `--web.route-prefix` | Префикс для внутренних маршрутов web-интерфейса |                                                                 |
| `--web.user-assets` | Путь к статическим ресурсам (`/user`) |                                                                 |
| `--web.enable-lifecycle` | Включить shutdown/reload через HTTP | `false`                                                         |
...
| `--storage.tsdb.retention` | [УСТАРЕЛО] Время хранения samples |                                                                 |
...
| `--storage.agent.wal-compression` | Сжатие WAL агента | `true`                                                          |
| `--storage.agent.retention.min-time` | Мин. время хранения samples (agent) |                                                                 |
| `--storage.agent.retention.max-time` | Макс. время хранения samples (agent) |                                                                 |
| `--storage.agent.no-lockfile` | Не создавать lock-файл (agent) | `false`                                                         |
| `--storage.remote.flush-deadline` | Таймаут сброса данных при shutdown | `1m`                                                            |
